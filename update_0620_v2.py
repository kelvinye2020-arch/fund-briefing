...
# -*- coding: utf-8 -*-
"""
基金行业资讯看板 2026-06-20 自动更新脚本 v2
- 不包含 emoji 字符，避免 PowerShell 编码问题
- 今天周六端午休市，更新 S0/S1/S2/S6/S7 模块
"""
...
print("[2/10] 更新 content-fingerprint")
...
<div class="stats-bar">
  ...
  <div class="stat-card">
    ...
    <div class="stat-change up">▲ 公募基金重大产品创新·主动管理+ETF结构融合·ETF行业进入主动化时代</div>
  </div>
  <div class="stat-card">
...
if old_stats in content:
    ...
    print("[4/10] 更新 Stats Bar")
else:
    print("[4/10] 警告: Stats Bar 旧内容未匹配，尝试继续...")

# ========== 5. 更新 S0 今日焦点 ==========
# 替换 S0 整个 section
old_s0_marker = '<!-- ============ Section 0: 今日焦点 ============ -->'
new_s0_marker = '<!-- ============ Section 0: 今日焦点 ============ -->'

old_s0_end = '<!-- ============ Section 1: 重磅信息 ============ -->'

if old_s0_marker in content and old_s0_end in content:
    s0_start = content.find(old_s0_marker)
    s0_end = content.find(old_s0_end)
    
    new_s0 = '''<!-- ============ Section 0: 今日焦点 ============ -->
  <div class="section">
    <div class="section-header">
      <div class="section-icon" style="background:#fef2f2;color:var(--danger);">FIRE</div>
    ...
    <div class="card-grid">
      <div class="card p0">
        <div class="card-top">
          <div class="card-title">RED 陆家嘴论坛6/18落幕！三大监管定调：不走注水救市老路·转向制度改革替代短期刺激</div>
          <div class="card-meta">
        ...
        <div class="card-body">
          ...
          <b>三大监管核心定调：</b>①<b>央行</b>：货币政策框架转向"资本成本锚定"，完善短端利率调控机制，推动低利率势能转化为实体融资与资产定价动能。高股息红利资产的"类债属性"将被重估。②<b>金融监管总局</b>：金融监管核心转向"风险出清"，中小金融机构"减量提质""硬约束"的改革方向明确，加速出清"僵尸"机构。③<b>证监会</b>：强化资本市场"制度闭环"建设，IPO端压紧压实全链条责任，退市端推动退市常态化，强化财务造假立体追责。<br>
        ...
        <div class="action-box">
          <div class="action-label">ACTION 腾安行动建议</div>
      ...
      <div class="card p0">
        <div class="card-top">
          <div class="card-title">RED 主动ETF获批！吴清6/17陆家嘴宣布·沪深交易所同步发布业务指引·公募基金重大产品创新</div>
          <div class="card-meta">
            <span class="priority-tag urgent">紧急必看</span>
        ...
        <div class="card-body">
          ...
          <b>行业意义：</b>主动ETF是2026年公募基金行业最具颠覆性的产品创新，将"主动管理投资能力"与"ETF交易便利"融合。美国主动ETF过去5年爆发式增长，中国正式加入这一赛道。<br>
      ...
      <div class="card p1">
        <div class="card-top">
          <div class="card-title">ORANGE 端午三市同休持续·节后6/22开市·美联储鹰派信号假期发酵·QDII产品节后或承压</div>
          <div class="card-meta">
  ...
  '''
    ...
    print("[5/10] 更新 S0 今日焦点")
else:
    print("[5/10] 警告: S0 段落标记未找到")

# ========== 6. 清理 S1 重磅信息中 T-14 之前的旧条目 ==========
print("[6/10] 清理 S1 重磅信息过期条目...")
# 删除 date-tag 06-05 的卡片（中证金牛）
s1_old_card = '''      <div class="card p1">
        <div class="card-top">
          <div class="card-title">ORANGE 中证金牛主动退场！招商/嘉实/兴证全球/汇添富等集体终止合作，第三方代销出清提速</div>
          <div class="card-meta">
            <span class="priority-tag important">重要关注</span>
...
if s1_old_card in content:
    content = content.replace(s1_old_card, '')
    print("  -> 删除 S1 中 06-05 过期条目（中证金牛）")
else:
    print("  -> 未找到 S1 06-05 条目（可能已删除）")

# ========== 7. 清理 S2 监管政策中 T-14 之前的旧条目 ==========
print("[7/10] 清理 S2 监管政策过期条目...")
# 删除 date-tag 06-05 的卡片（国办私募基金指导意见）
s2_old_card = '''      <div class="card p0">
        <div class="card-top">
          <div class="card-title">RED 国办发文：23万亿私募基金迎顶层设计！全链条严监管+三年行动方案，出清5444家管理人</div>
          <div class="card-meta">
            <span class="priority-tag urgent">紧急必看</span>
            <span class="date-tag">06-05</span>
          </div>
...
if s2_old_card in content:
    content = content.replace(s2_old_card, '')
    print("  -> 删除 S2 中 06-05 过期条目（国办私募基金）")
else:
    print("  -> 未找到 S2 06-05 条目（可能已删除）")

# ========== 8. 更新 S6 市场行情速览 ==========
print("[8/10] 更新 S6 市场行情速览...")
...
new_s6 = '''    <div class="card p3">
      ...
      <div class="card-body">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          <div>
            <b>MARKET 节前最后交易日（6/18）收盘：</b>A股分化，沪指<b>4090点(-0.43%)</b>，深证成指<b>16030点(+0.94%)</b>，创业板指<b>4252点(+2.05%)</b>，科创50指数<b>涨3.84%</b>。芯片/半导体强者恒强。全市场成交<b>3.31万亿</b>。<br><br>
            <b>MARKET 港股（6/18）：</b>恒生指数<b>跌1.59%</b>，恒生科技<b>跌1.39%</b>，美联储鹰派预期压制港股科技。<br><br>
            <b>MARKET 端午休市：</b>6/19-6/21休市，<b>6/22（周一）开市</b>。三市同休历史罕见。
          </div>
          <div>
            <b>MARKET 周末解读焦点：</b><br>
            ▪ 陆家嘴论坛闭幕，三大监管定调持续发酵<br>
            ▪ 央行：货币政策转向资本成本锚定<br>
            ▪ 金融监管总局：风险出清+减量提质<br>
            ▪ 证监会：制度闭环+退市常态化<br><br>
            <b>MARKET 节后关注（6/22）：</b><br>
...
# ========== 9. 更新 S7 时间线 ==========
print("[9/10] 更新 S7 时间线...")
# 删除 06-06 的时间线条目（中基协换届）
old_tl_0606 = '''      <div class="timeline-item">
...
if old_tl_0606 in content:
    content = content.replace(old_tl_0606, '')
    print("  -> 删除 S7 中 06-06 过期时间线条目")
else:
    print("  -> 未找到 S7 06-06 条目（可能已删除）")

# 添加 06-20 时间线条目
new_tl_0620 = '''      <div class="timeline-item">
        <div class="timeline-dot red"></div>
        <div>
          <div class="timeline-date">2026-06-20（陆家嘴论坛闭幕解读·三大监管定调持续发酵·主动ETF获批行业热议·端午休市中）</div>
...
# 在时间线区域的第一个 timeline-item 之前插入新条目
timeline_marker = '<!-- ============ Section 7: 关键时间线 ============ -->'
if timeline_marker in content:
    tl_start = content.find(timeline_marker)
    first_item_idx = content.find('<div class="timeline-item">', tl_start)
    if first_item_idx > 0:
        # 在第一个 timeline-item 之前插入新条目
        content = content[:first_item_idx] + new_tl_0620 + '\n' + content[first_item_idx:]
